...
<aside
    ...
    @mouseenter="if(collapseTimeout) { clearTimeout(collapseTimeout); collapseTimeout = null }; hoverTimeout = setTimeout(() => { if(!mobileOpen) expandOnHover() }, 200)"
    @mouseleave="if(hoverTimeout) { clearTimeout(hoverTimeout); hoverTimeout = null }; if(!mobileOpen) collapseTimeout = setTimeout(() => collapseOnLeave(), 300)"
    x-data="{ hoverTimeout: null, collapseTimeout: null }"
    role="navigation"
    aria-label="Main navigation sidebar"
>
    <div class="h-full flex flex-col overflow-hidden">
        <!-- Logo -->
        <div class="h-24 flex-shrink-0 flex items-center justify-center px-4 border-b border-[var(--surface-glass-border)]">
            <a href="{{ route('dashboard') }}" class="flex flex-col items-center gap-2 py-3 w-full">
                <div
                    class="relative transition-all duration-300"
                    :class="expanded ? 'h-14 w-14' : 'h-10 w-10'"
                >
                    <img
                        src="{{ asset('images/logo kemenkes.png') }}"
                        alt="Logo Kemenkes"
                        class="absolute inset-0 h-full w-full object-contain transition-opacity duration-200"
                        :class="(!expanded || isAnimating) ? 'opacity-100' : 'opacity-0'"
                    >
                    <img
                        src="{{ asset('images/Logo-RS-New.png') }}"
                        alt="Logo RS Ngoerah"
                        class="absolute inset-0 h-full w-full scale-110 object-contain transition-opacity duration-200"
                        :class="(expanded && !isAnimating) ? 'opacity-100' : 'opacity-0'"
                    >
                </div>
            </a>
        ...
        <!-- Sidebar Footer -->
        <div class="flex-shrink-0 border-t border-[var(--surface-glass-border)] p-4">
            <button
                @click="toggle()"
